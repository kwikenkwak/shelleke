 ...
 * — click one to get its terminal and tabs back without touching the working copies.
...
PixPanel {
    ...
    // Selected repos that carry a package.json: they get `pnpm install` after checkout and
    // a SESSION_SECRET in front of their vitulina command.
    ...
    ColumnLayout {
        ...
        GridLayout {
            ...
            PixText {
                Layout.fillWidth: true
                text: "jj workspace, copy .env, direnv allow"
                    + (root.nodeRepos.length > 0
                        ? ", pnpm i (" + root.nodeRepos.join(", ") + ")"
                        : "")
                font.pixelSize: PixTheme.font.pixelSize.smaller
                elide: Text.ElideRight
                wrapMode: Text.WordWrap
        ...
        RowLayout {
            ...
            PixButton {
                ...
                PixText {
                    anchors.centerIn: parent
                    text: Worktrees.busy ? "WORKING…" : "CREATE"
                    font.bold: true
        ...
        RowLayout {
            ...
            PixText {
                text: Worktrees.tasks.length + " task" + (Worktrees.tasks.length === 1 ? "" : "s")
                color: PixTheme.colors.grey2
                font.pixelSize: PixTheme.font.pixelSize.smaller
            }
        }
